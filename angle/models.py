...
class aAngleModel(object):
    """
    the attentional angle model
    """

    def __init__(self, mu_list, t_list, sigma=1) -> None:
        """
    ...
    def drift_coeff(self, _X: float, t: float) -> float:
        _t_list = np.insert(self.t_list, 0, 0)
        _t_list = np.append(_t_list, np.inf)
        return np.piecewise(t, [(t >= _t_list[i]) & (t <= _t_list[i + 1]) for i in range(self.d)], self.mu_list)
    ...
    def Milstein(self, init_cond, T, Nt=1000):
        ...
        for i in range(1, Nt + 1):
            t = t0 + (i - 1) * dt
            X = X_grid[i - 1]
            X_grid[i] = X + self.drift_coeff(X, t) * dt + self.diffu_coeff(X, t) * self.dW(dt) + 0.5 * self.diffu_coeff(X, t) ** 2 * (self.dW(dt) ** 2 - dt)
    ...
    def simulate_FPTD(self, init_cond, T, a, theta, dt=0.001, num=3000):
        ...
        for n in range(num):
            ...
            for i in range(1, int((T - t0) / dt) + 10):
                t = t0 + (i - 1) * dt
                X += self.drift_coeff(X, t) * dt + self.diffu_coeff(X, t) * self.dW(dt) + 0.5 * self.diffu_coeff(X, t) ** 2 * (self.dW(dt) ** 2 - dt)
                if X >= upper_bdy(t):
                    upper_time.append(t)
                    break
                elif X <= lower_bdy(t):
...
class AngleModel(object):
    ...
    def EulerMaruyama(self, init_cond, T, Nt=1000):
        ...
        for i in range(1, Nt + 1):
            t = t0 + (i - 1) * dt
            X = X_grid[i - 1]
            X_grid[i] = X + self.drift_coeff(X, t) * dt + self.diffu_coeff(X, t) * self.dW(dt)
        return t_grid, X_grid
    ...
    def LeimkuhlerMatthews(self, init_cond, T, Nt=1000):
        ...
        for i in range(1, Nt + 1):
            t = t0 + (i - 1) * dt
            X = X_grid[i - 1]
            X_temp = X + self.drift_coeff(X, t) * dt + 0.5 * self.diffu_coeff(X, t) * self.dW(dt)
